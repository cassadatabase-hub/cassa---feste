...
function CassaContent() {
  const { t } = useLang();
  const { categories, products, comandaTemplates, settings, loading } = useCatalog();
      ...
      <main className="max-w-6xl mx-auto px-4 py-4">
        <Tabs defaultValue="lookup" className="w-full">
          ...
          <TabsContent value="lookup">
            <CodeLookup categories={categories} comandaTemplates={comandaTemplates} onOrderCompleted={() => setRefreshKey(k => k + 1)} />
          </TabsContent>
          <TabsContent value="manual">
            <ManualOrder categories={categories} products={products} comandaTemplates={comandaTemplates} />
          </TabsContent>
          <TabsContent value="history">
            <OrderHistory categories={categories} comandaTemplates={comandaTemplates} refreshKey={refreshKey} activeFestaId={settings?.active_festa_id} />
          </TabsContent>
        </Tabs>
      </main>
    </div>
